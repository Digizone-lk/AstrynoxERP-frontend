...
export default function AuditLogPage() {
  ...
  useEffect(() => {
    if (user && !canFinance(user.role)) router.replace("/ims/dashboard");
  }, [user, router]);
  ...
  return (
    <div>
      ...
      {isLoading ? (
        <div className="bg-white rounded-xl border border-slate-200 px-5 py-4"><ActivitySkeleton count={10} /></div>
      ) : logs.length === 0 ? (
