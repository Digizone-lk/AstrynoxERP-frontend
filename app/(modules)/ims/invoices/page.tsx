...
export default function InvoicesPage() {
  ...
  return (
    <div>
      ...
      {isLoading ? (
        <ListSkeleton title="w-24" />
      ) : invoices.length === 0 ? (
        <div className="text-center py-16 text-slate-400">
          <Receipt size={40} className="mx-auto mb-3 opacity-40" />
          <p>No invoices yet. {canWrite && <Link href="/ims/invoices/new" className="text-blue-600 hover:underline">Create one</Link>}</p>
        </div>
      ) : (
        <div className="space-y-3">
          {invoices.map((inv) => (
            <Link key={inv.id} href={`/ims/invoices/${inv.id}`}>
