...
import { ListSkeleton } from "@/components/ims/skeletons";
...
export default function QuotationsPage() {
  ...
  return (
    <div>
      ...
      {isLoading ? (
        <ListSkeleton title="w-32" />
      ) : quotations.length === 0 ? (
        <div className="text-center py-16 text-slate-400">
          <FileText size={40} className="mx-auto mb-3 opacity-40" />
          <p>No quotations yet. {canWrite && <Link href="/ims/quotations/new" className="text-blue-600 hover:underline">Create one</Link>}</p>
        </div>
      ) : (
        <div className="space-y-3">
          {quotations.map((q) => (
            <Link key={q.id} href={`/ims/quotations/${q.id}`}>
